...
#txt = input("enter your id")
#txt2 = str(input("enter your name"))
...
#def update():
 #g.db = connect_db()
 #g.db.execute("UPDATE attendance SET ATTENDANCE = 'PRESENT' WHERE NAME = 'shailja'")
 #g.db.commit()
...
@login_required
def add():
 t1=str(input("enter your name"))
 g.db=connect_db()
        
 g.db.execute('INSERT INTO students (id,name,class,emailid,phoneno,rollno) VALUES(?,?,?,?,?,?)',[request.form['id'],request.form['name'],request.form['class'],request.form['emailid'],request.form['phoneno'],request.form['rollno']]);
 g.db.execute("UPDATE attendance SET ATTENDANCE = 'PRESENT' WHERE NAME =monika")
 g.db.commit()
 flash('posted')
 #return redirect(url_for('home'))
 return redirect(url_for('home'))
